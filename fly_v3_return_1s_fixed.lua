...
local function restartTPWalking()
    if not nowe then
        return
    end
    ...
    for i = 1, speeds do
        task.spawn(function()
            ...
            while tpwalking and character.Parent and humanoid.Parent and humanoid.Health > 0 do
                hb:Wait()
...
-- Usa MouseButton1Click para garantir o toque no mobile.
...
speaker.CharacterAdded:Connect(function(character)
    ...
    -- Mantém a posição salva, mas faz o personagem olhar para a direção
    -- atual da câmera ao voltar.
    if hasReturn then
        ...
        if camera then
            ...
            if flatLook.Magnitude > 0.001 then
                ...
                target = CFrame.lookAt(target.Position, target.Position + flatLook)
            end
        end
    end
